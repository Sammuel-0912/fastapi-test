def test_read_machines_empty(client):
    response = client.get("/machines/")
    assert response.status_code == 200
    assert response.json() == []  # 預期剛開始應該是空陣列


def test_create_machine_without_token(client):
...
def test_full_auth_and_create_machine_flow(client):
    """測試完整流程：註冊 -> 登入取得 Token -> 成功建立機台"""

    # 1.註冊新帳號
    auth_payload = {"username": "testuser", "password": "testpassword"}
    reg_response = client.post("/auth/register", json=auth_payload)
    assert reg_response.status_code == 201

    # 2.登入並取得Token(注意：OAuth2 表單格式要用 data= 而不是 json=)
    login_response = client.post("/auth/login", data=auth_payload)
    ...
    create_response = client.post("/machines/", json=machine_payload, headers=headers)
