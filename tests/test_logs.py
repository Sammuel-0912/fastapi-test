...
@pytest.mark.asyncio
async def test_create_and_read_nested_logs(client):
    # 🔑 1. 註冊與登入以取得 JWT Token
    auth_payload = {"username": "Ben", "password": "123456"}
    ...
    # 1. 先建立一台機台
    m_resp = await client.post("/machines/", json={"name": "Test-Machine", "model": "M1"},headers=headers)
    ...
    create_resp = await client.post(f"/machines/{machine_id}/logs/", json=log_data)
    assert create_resp.status_code == 201
    assert create_resp.json()["machine_id"] == machine_id

    # 3. 測試 GET /machines/{machine_id}/logs
    get_resp = await client.get(f"/machines/{machine_id}/logs/")
